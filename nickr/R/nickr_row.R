...
nickr_row <- function(.data, cond, msg = "nickr_row", active = TRUE, logger = stop) {
  ...
  if (active) {
    ...
    # Augment with row index as `.r`.
    augmented <- tibble::rowid_to_column(.data, ".r")

    # Check that each row passes condition.
    cond <- rlang::enquo(cond)
    passes <- purrr::pmap_lgl(augmented, function(...) {
      # Augment arguments with tibble row as `.row` (which doesn't have a row ID in `.r`).
      func_args <- list(...)
    ...
    # Report. (Negating once to create failures makes the code easier to read.)
    failures <- !passes
    if (any(failures)) {
